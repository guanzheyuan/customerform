...
@Form.create()
class ItemTargetManage extends PureComponent {
  state = {
    expandForm: false,
    selectedRows: [],
    formValues: {},
  ...
  handleDeleteModalVisible = (flag, record) => {
    const { dispatch, form } = this.props;
    dispatch({
      type: 'rule/deleteProjectTargetYear',
      payload: record,
      callback: response => {
        if (response === 1) {
          form.validateFields((err, fieldsValue) => {
  ...
  handleEditModalVisible = (flag, record) => {
    const pageState = 1;
    router.push({
      pathname: '/Economy/AddItemTargetManage',
      record,
      pageState,
    });
  };
  ...
  handleModalVisible = () => {
    router.push('/Economy/AddItemTargetManage');
  };
  ...
  render() {
    ...
    return (
      <PageHeaderWrapper title="项目目标管理">
        <Card bordered={false}>
...
export default ItemTargetManage;
